...
use App\Mail\MeetingReservedMail;
use App\User;
use App\Models\Cart;
...
use MeetingReservedNotification;
...
class MeetingController extends Controller
{
    ...
    {
        // Mail::raw('This is a test email content', function ($message) {
        //     $message->to('munirahmed4friends@gmail.com')
        //             ->subject('Test Email');
        // });
        // return $request;
        $user = auth()->user();
        // return $user;
        // dd($request->instructor_email);
        $this->instructorEmail = $request->instructor_email;
        $this->studentMessage = $request->description;
        $this->studentName = $user->full_name;
        $this->instructor = User::where('email', $request->instructor_email)->first();
            ...
            if (!empty($timeId)) {
                ...
                if (!empty($meetingTime)) {
                    ...
                    $details = [
                        ...
                        'reservation_time' => $meetingTime['time'] ?? ''
                    ];
                    // dd($details);
                    // $check = Mail::to($this->instructorEmail)->send(new MeetingReservedMail($details));
                    $check = Mail::to(env('TEACHER_EMAIL'))->send(new MeetingReservedMail($details));

                    // // Get all admin users with role_id = 1
                    // $adminUsers = User::where('role_id', 1)->get();

                    // // Loop through all admins and send them the email
                    // foreach ($adminUsers as $adminUser) {
                    //     Mail::to($adminUser->email)->send(new MeetingReservedMail($details));
                    // }
                    ...
                    if (!empty($meeting) and !$meeting->disabled) {
                        if (!empty($meeting->amount) and $meeting->amount > 0) {
                            ...
                            Mail::to($user->email)->send(new MeetingReservedMail());
                            return response()->json($toastData);
                        } else {
