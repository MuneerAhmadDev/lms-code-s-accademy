    ...
    <div class="col-12 col-md-5">
        ...
        <div class="form-group mt-15 ">
            ...
            <select name="type" class="custom-select @error('type')  is-invalid @enderror">
                ...
                <option value="text_lesson" @if (
                    !empty($upcomingCourse) and $upcomingCourse->type == 'text_lesson' or
                        old('type') == \App\Models\Webinar::$textLesson) selected @endif>
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="title"
                value="{{ !empty($upcomingCourse) ? $upcomingCourse->title : old('title') }}"
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="slug"
                value="{{ !empty($upcomingCourse) ? $upcomingCourse->slug : old('slug') }}"
        ...
        @if (!empty($upcomingCourse) and $upcomingCourse->creator->isOrganization())
            <div class="form-group mt-15 ">
                ...
                <select name="organ_id" data-search-option="just_organization_role"
                    class="form-control search-user-select2" data-placeholder="{{ trans('search_organization') }}">
                    <option value="{{ $upcomingCourse->creator->id }}" selected>
                        {{ $upcomingCourse->creator->full_name }}</option>
                </select>
            </div>
        @endif
        ...
        <div class="form-group mt-15 ">
            ...
            <select name="teacher_id" data-search-option="except_user" class="form-control search-user-select22"
                ...
                @if (!empty($upcomingCourse))
                    <option value="{{ $upcomingCourse->teacher->id }}" selected>
                        {{ $upcomingCourse->teacher->full_name }}</option>
                @else
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="seo_description"
                value="{{ !empty($upcomingCourse) ? $upcomingCourse->seo_description : old('seo_description') }}"
        ...
        <div class="form-group mt-15">
            ...
            <div class="input-group">
                <div class="input-group-prepend">
                    <button type="button" class="input-group-text admin-file-manager" data-input="thumbnail"
                        data-preview="holder">
                        <i class="fa fa-upload"></i>
                    </button>
                </div>
                <input type="text" name="thumbnail" id="thumbnail"
                    value="{{ !empty($upcomingCourse) ? $upcomingCourse->thumbnail : old('thumbnail') }}"
        ...
        <div class="form-group mt-15">
            ...
            <div class="input-group">
                <div class="input-group-prepend">
                    <button type="button" class="input-group-text admin-file-manager" data-input="cover_image"
                        data-preview="holder">
                        <i class="fa fa-upload"></i>
                    </button>
                </div>
                <input type="text" name="image_cover" id="cover_image"
                    value="{{ !empty($upcomingCourse) ? $upcomingCourse->image_cover : old('image_cover') }}"
            ...
            <div class="">
                ...
                <select name="video_demo_source" class="js-video-demo-source form-control">
                    @foreach (\App\Models\Webinar::$videoDemoSource as $source)
                        <option value="{{ $source }}" @if (!empty($upcomingCourse) and $upcomingCourse->video_demo_source == $source) selected @endif>
        ...
        <div
            class="js-video-demo-other-inputs form-group mt-0 {{ (empty($upcomingCourse) or $upcomingCourse->video_demo_source != 'secure_host') ? '' : 'd-none' }}">
            <label class="input-label font-12">{{ trans('update.path') }}</label>
            <div class="input-group js-video-demo-path-input">
                <div class="input-group-prepend">
                    <button type="button"
                        class="js-video-demo-path-upload input-group-text admin-file-manager {{ (empty($upcomingCourse) or empty($upcomingCourse->video_demo_source) or $upcomingCourse->video_demo_source == 'upload') ? '' : 'd-none' }}"
                        data-input="demo_video" data-preview="holder">
                        <i class="fa fa-upload"></i>
                    </button>

                    <button type="button"
                        class="js-video-demo-path-links rounded-left input-group-text input-group-text-rounded-left  {{ (empty($upcomingCourse) or empty($upcomingCourse->video_demo_source) or $upcomingCourse->video_demo_source == 'upload') ? 'd-none' : '' }}">
                        <i class="fa fa-link"></i>
                    </button>
                </div>
                <input type="text" name="video_demo" id="demo_video"
                    value="{{ !empty($upcomingCourse) ? $upcomingCourse->video_demo : old('video_demo') }}"
        ...
        <div
            class="form-group js-video-demo-secure-host-input {{ (!empty($upcomingCourse) and $upcomingCourse->video_demo_source == 'secure_host') ? '' : 'd-none' }}">
            <div class="input-group">
                <div class="input-group-prepend">
                ...
                <div class="custom-file js-ajax-s3_file">
                    <input type="file" name="video_demo_secure_host_file" class="custom-file-input cursor-pointer"
...
<div class="row">
    <div class="col-12">
        <div class="form-group mt-15">
            <label class="input-label">{{ trans('public.description') }} in English</label>
            <textarea id="summernote" name="description" class="form-control @error('description')  is-invalid @enderror"
                placeholder="{{ trans('forms.webinar_description_placeholder') }}">{!! !empty($upcomingCourse) ? $upcomingCourse->description : old('description') !!}</textarea>
            @error('description')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
    ...
    <div class="col-12">
        <div class="form-group mt-15">
            <label class="input-label">{{ trans('public.description') }} in German</label>
            <textarea id="summernote_german" name="german_description"
                class="form-control @error('description')  is-invalid @enderror"
                placeholder="{{ trans('forms.webinar_description_placeholder') }}">{!! !empty($upcomingCourse) ? $upcomingCourse->german_description : old('german_description') !!}</textarea>
            @error('description')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
